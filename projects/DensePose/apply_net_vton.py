...
class ShowAction(InferenceAction):
 ...
 #       logger.info(f"Processing {image_fpath}")
        ...
        # put here a black image with the size of the original image
        image_np = np.array(image)
        height, width, channel = image_np.shape # get the size of the original image
        black_image = Image.new('RGB', (width, height), 'black') # make a black image
        black_image_np = np.array(black_image) # turn it into a numpy array
...
#        out_fname = cls._get_out_fname(entry_idx, context["out_fname"])
        ...
        out_dir = os.path.dirname(out_fname)
        if len(out_dir) > 0 and not os.path.exists(out_dir): os.makedirs(out_dir)
